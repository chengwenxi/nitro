...
The following is a summary of changes in the Mantle One chain's 8/31/22 Nitro upgrade that dapp devs should be aware of.
...
- **Tighter Syncronization with L1 Block Numbers 🥳**: L1 block numbers (accessed via `block.number` on L2) are updated more frequently in Nitro than in Mantle classic; expect them to be nearly real-time/ in sync with L1.
...
- **Retryable Tickets**:
  ...
  - For the redemption of retryable tickets, the calculation of the L2 transaction ID changed and differs between different redeem attempts (i.e. after failed attempts). See [mantle-sdk](https://github.com/offchainlabs/mantle-sdk/tree/c-nitro-stable) for a reference implementation on the new client-side flow.
...
- **MtSys precompile**: `MtSys.getTransactionCount` and `MtSys.getStorageAt` are removed in nitro
...
- **Outbox Changes**: The Outbox has a new (simplified!) architecture; in short, all outgoing messages will be included in a single Merkle tree (opposed to Mantle classic, in which many outbox entries, each with its own Merkle root). See [mantle-sdk](https://github.com/offchainlabs/mantle-sdk/tree/c-nitro) for a reference implementation on how to handle the new flow of interacting with the outbox.
