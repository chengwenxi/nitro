...
func TestMtAggregatorBatchPosters(t *testing.T) {
	...
	bps, err := MtAggregator{}.GetBatchPosters(context, evm)
	Require(t, err)
	if len(bps) != 1 {
		Fail(t)
	}
	...
	Require(t, MtDebug{}.BecomeChainOwner(context, evm))
	Require(t, MtAggregator{}.AddBatchPoster(context, evm, addr))
