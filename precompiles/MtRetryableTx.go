...
func (con MtRetryableTx) oldNotFoundError(c ctx) error {
...
func (con MtRetryableTx) Redeem(c ctx, evm mech, ticketId bytes32) (bytes32, error) {
	...
	writeBytes := mtmath.WordsForBytes(byteCount)
	...
	return retryTxHash, c.State.L2PricingState().AddToGasPool(mtmath.SaturatingCast(gasToDonate))
}

// Gets the default lifetime period a retryable has at creation
func (con MtRetryableTx) GetLifetime(c ctx, evm mech) (huge, error) {
	return big.NewInt(retryables.RetryableLifetimeSeconds), nil
}

// Gets the timestamp for when ticket will expire
func (con MtRetryableTx) GetTimeout(c ctx, evm mech, ticketId bytes32) (huge, error) {
	retryableState := c.State.RetryableState()
	retryable, err := retryableState.OpenRetryable(ticketId, evm.Context.Time.Uint64())
	if err != nil {
		return nil, err
...
func (con MtRetryableTx) Keepalive(c ctx, evm mech, ticketId bytes32) (huge, error) {
	...
	updateCost := mtmath.WordsForBytes(nbytes) * params.SstoreSetGas / 100
...
func (con MtRetryableTx) GetBeneficiary(c ctx, evm mech, ticketId bytes32) (addr, error) {
	retryableState := c.State.RetryableState()
	retryable, err := retryableState.OpenRetryable(ticketId, evm.Context.Time.Uint64())
	if err != nil {
		return addr{}, err
...
func (con MtRetryableTx) Cancel(c ctx, evm mech, ticketId bytes32) error {
	if c.txProcessor.CurrentRetryable != nil && ticketId == *c.txProcessor.CurrentRetryable {
...
func (con MtRetryableTx) GetCurrentRedeemer(c ctx, evm mech) (common.Address, error) {
...
func (con MtRetryableTx) SubmitRetryable(
