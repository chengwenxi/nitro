...
func TestMtAddressTableInit(t *testing.T) {
	evm := newMockEVMForTesting()
	mtab := MtAddressTable{}
	context := testContext(common.Address{}, evm)

	size, err := mtab.Size(context, evm)
	Require(t, err)
	if (!size.IsInt64()) || (size.Int64() != 0) {
		t.Fatal()
	}

	_, shouldErr := mtab.Lookup(context, evm, common.Address{})
	if shouldErr == nil {
		t.Fatal()
	}

	_, shouldErr = mtab.LookupIndex(context, evm, big.NewInt(0))
	if shouldErr == nil {
		t.Fatal()
	}
}
...
func TestAddressTable1(t *testing.T) {
	...
	// register addr
	slot, err := mtab.Register(context, evm, addr)
	Require(t, err)
	...
	index, err := mtab.Lookup(context, evm, addr)
	Require(t, err)
	...
	addr2, err := mtab.LookupIndex(context, evm, big.NewInt(0))
	Require(t, err)
	if addr2 != addr {
		t.Fatal()
	}
	...
	_, shouldErr = mtab.LookupIndex(context, evm, big.NewInt(1))
	if shouldErr == nil {
		t.Fatal()
	}
}
...
func TestAddressTableCompressNotInTable(t *testing.T) {
	...
	res, err := mtab.Compress(context, evm, addr)
	Require(t, err)
	if len(res) != 21 {
		t.Fatal()
	}
	...
	dec, nbytes, err := mtab.Decompress(context, evm, res, big.NewInt(0))
	Require(t, err)
...
func TestAddressTableCompressInTable(t *testing.T) {
	...
	// Register addr
	if _, err := mtab.Register(context, evm, addr); err != nil {
		t.Fatal(err)
	}
	...
	dec, nbytes, err := mtab.Decompress(context, evm, res, big.NewInt(1))
	Require(t, err)
...
func newMockEVMForTestingWithVersion(version *uint64) *vm.EVM {
	...
	if version != nil {
		chainConfig.MantleChainParams.InitialMtOSVersion = *version
	}
	...
	evm.ProcessingHook = &mtos.TxProcessor{}
	return evm
}
