...
var testChainConfig = &params.ChainConfig{
	...
	MantleChainParams:   params.MantleDevTestParams(),
}
...
func TestEthDepositMessage(t *testing.T) {
	...
	header := mtos.L1IncomingMessageHeader{
		Kind:        mtos.L1MessageType_EthDeposit,
		Poster:      addr,
		BlockNumber: 864513,
...
func RunMessagesThroughAPI(t *testing.T, msgs [][]byte, statedb *state.StateDB) {
	...
	for _, data := range msgs {
		msg, err := mtos.ParseIncomingL1Message(bytes.NewReader(data))
		if err != nil {
			t.Error(err)
		}
		...
		mtos.FinalizeBlock(nil, nil, statedb)
	}
}
