...
func TestBatchPosterParallel(t *testing.T) {
	...
	l2info, nodeA, l2clientA, l2stackA, l1info, _, l1client, l1stack := createTestNodeOnL1WithConfig(t, ctx, true, conf, params.MantleDevTestChainConfig())
	defer requireClose(t, l1stack)
	defer requireClose(t, l2stackA)

	l2clientB, _, l2stackB := Create2ndNode(t, ctx, nodeA, l1stack, &l2info.MtInitData, nil)
	...
	for i := 0; i < parallelBatchPosters; i++ {
		batchPoster, err := mtnode.NewBatchPoster(nodeA.L1Reader, nodeA.InboxTracker, nodeA.TxStreamer, nodeA.SyncMonitor, func() *mtnode.BatchPosterConfig { return &conf.BatchPoster }, nodeA.DeployInfo.SequencerInbox, &seqTxOpts, nil)
		Require(t, err)
	...
	if false {
		...
		Require(t, err)
		seqInbox, err := mtnode.NewSequencerInbox(l1client, nodeA.DeployInfo.SequencerInbox, 0)
		Require(t, err)
