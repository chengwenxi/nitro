...
func NewInboxReader(tracker *InboxTracker, client mtutil.L1Interface, l1Reader *headerreader.HeaderReader, firstMessageBlock *big.Int, delayedBridge *DelayedBridge, sequencerInbox *SequencerInbox, config InboxReaderConfigFetcher) (*InboxReader, error) {
	err := config().Validate()
	if err != nil {
		return nil, err
	}
...
func (ir *InboxReader) run(ctx context.Context, hadError bool) error {
	...
	for {
		...
		neededBlockAdvance := config.DelayBlocks + mtmath.SaturatingUSub(config.MinBlocksToRead, 1)
		neededBlockHeight := mtmath.BigAddByUint(from, neededBlockAdvance)
		checkDelayTimer := time.NewTimer(config.CheckDelay)
	WaitForHeight:
		for mtmath.BigLessThan(currentHeight, neededBlockHeight) {
			select {
		...
		if !missingDelayed && !reorgingDelayed && !missingSequencer && !reorgingSequencer {
			...
			from = mtmath.BigAddByUint(currentHeight, 1)
...
func (r *InboxReader) getPrevBlockForReorg(from *big.Int) (*big.Int, error) {
	...
	newFrom := mtmath.BigSub(from, big.NewInt(10))
...
func (r *InboxReader) getNextBlockToRead() (*big.Int, error) {
	...
	if mtmath.BigLessThan(msgBlock, r.firstMessageBlock) {
