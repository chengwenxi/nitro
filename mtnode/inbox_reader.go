...
func NewInboxReader(tracker *InboxTracker, client mtutil.L1Interface, l1Reader *headerreader.HeaderReader, firstMessageBlock *big.Int, delayedBridge *DelayedBridge, sequencerInbox *SequencerInbox, config *InboxReaderConfig) (*InboxReader, error) {
	err := config.Validate()
	if err != nil {
		return nil, err
	}
...
func (ir *InboxReader) run(ctx context.Context, hadError bool) error {
	...
	neededBlockAdvance := ir.config.DelayBlocks + mtmath.SaturatingUSub(ir.config.MinBlocksToRead, 1)
