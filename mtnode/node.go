...
func DeployOnL1(ctx context.Context, l1client mtutil.L1Interface, deployAuth *bind.TransactOpts, sequencer common.Address, authorizeValidators uint64, readerConfig headerreader.ConfigFetcher, machineConfig validator.MantleMachineConfig, config rollupgen.Config) (*RollupAddresses, error) {
...
func ConfigAddOptions(prefix string, f *flag.FlagSet, feedInputEnable bool, feedOutputEnable bool) {
	mantle.ConfigAddOptions(prefix+".rpc", f)
...
func createNodeImpl(
	...
	if err != nil {
		if l2Config.MantleChainParams.GenesisBlockNum > 0 {
	...
	arbInterface, err := NewMtInterface(txStreamer, txPublisher)
	if err != nil {
		return nil, err
	}
	backend, err := mantle.NewBackend(stack, &config.RPC, chainDb, arbInterface, syncMonitor)
	if err != nil {
		return nil, err
	}
	...
	} else if l2BlockChain.Config().MantleChainParams.DataAvailabilityCommittee {
	...
	if blockValidatorConf.Enable && !(blockValidatorConf.MtitratorValidator || blockValidatorConf.JitValidator) {
		log.Warn("No block-by-block validator configured. Enabling the JIT block validator")
		blockValidatorConf.JitValidator = true
	}

	nitroMachineConfig := validator.DefaultMantleMachineConfig
	...
	nitroMachineLoader := validator.NewMantleMachineLoader(nitroMachineConfig, fatalErrChan)
...
func CreateNode(
	...
	apis = append(apis, rpc.API{
		...
		Service: &MtDebugAPI{
			blockchain:        l2BlockChain,
			blockRangeBound:   config.RPC.MtDebug.BlockRangeBound,
			timeoutQueueBound: config.RPC.MtDebug.TimeoutQueueBound,
		},
		Public: false,
	})
...
func WriteOrTestGenblock(chainDb ethdb.Database, initData statetransfer.InitDataReader, chainConfig *params.ChainConfig, accountsPerSync uint) error {
	...
	stateRoot, err := mtosState.InitializeMtosInDatabase(chainDb, initData, chainConfig, timestamp, accountsPerSync)
	if err != nil {
		return err
	}

	genBlock := mtosState.MakeGenesisBlock(prevHash, blockNumber, timestamp, stateRoot, chainConfig)
