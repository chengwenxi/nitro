...
import (
	...
	"github.com/mantlenetworkio/mantle/mtnode/dataposter"
...
type buildingBatch struct {
	...
	startMsgCount mtutil.MessageIndex
	msgCount      mtutil.MessageIndex
}
...
func (s *batchSegments) addSegment(segment []byte, isHeader bool) (bool, error) {
	...
	if overflow || len(s.rawSegments) >= mtstate.MaxSegmentsPerSequencerMessage {
...
func (s *batchSegments) addL2Msg(l2msg []byte) (bool, error) {
	...
	segment[0] = mtstate.BatchSegmentKindL2Message
...
func (s *batchSegments) addDelayedMessage() (bool, error) {
	segment := []byte{mtstate.BatchSegmentKindDelayedMessages}
...
func (s *batchSegments) AddMessage(msg *mtstate.MessageWithMetadata) (bool, error) {
	if s.isDone {
		return false, errBatchAlreadyClosed
	}
	...
	success, err := s.maybeAddDiffSegment(&s.timestamp, msg.Message.Header.Timestamp, mtstate.BatchSegmentKindAdvanceTimestamp)
	if !success {
		return false, err
	}
	success, err = s.maybeAddDiffSegment(&s.blockNum, msg.Message.Header.BlockNumber, mtstate.BatchSegmentKindAdvanceL1BlockNumber)
	if !success {
		return false, err
	}
...
func (s *batchSegments) CloseAndGetBytes() ([]byte, error) {
	...
	fullMsg[0] = mtstate.BrotliMessageHeaderByte
	fullMsg = append(fullMsg, compressedBytes...)
	return fullMsg, nil
}

func (b *BatchPoster) encodeAddBatch(seqNum *big.Int, prevMsgNum mtutil.MessageIndex, newMsgNum mtutil.MessageIndex, message []byte, delayedMsg uint64) ([]byte, error) {
...
func (b *BatchPoster) maybePostSequencerBatch(ctx context.Context) error {
	...
	for b.building.msgCount < msgCount {
		...
		if msg.Message.Header.Kind != mtos.L1MessageType_BatchPostingReport {
