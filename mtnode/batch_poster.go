...
func (b *BatchPoster) maybePostSequencerBatch(ctx context.Context, batchSeqNum uint64) (*types.Transaction, error) {
	...
	if !mtmath.BigEquals(inboxContractCount, mtmath.UintToBig(batchSeqNum)) {
		...
		if timeSinceNextMessage <= time.Minute && mtmath.BigEquals(inboxContractCount, mtmath.UintToBig(batchSeqNum+1)) {
			return nil, nil
		}
	...
	if b.config.HighGasThreshold != 0 && tx.GasFeeCap().Cmp(highGasThreshold) >= 0 && timeSinceNextMessage < b.config.HighGasDelay {
		...
		if lastHeader.BaseFee.Cmp(highGasThreshold) >= 0 {
			...
			tx = types.NewTx(&types.DynamicFeeTx{
				...
				GasFeeCap:  mtmath.BigMulByFrac(highGasThreshold, 6, 5),
				Gas:        tx.Gas(),
				To:         tx.To(),
				Value:      tx.Value(),
				Data:       tx.Data(),
