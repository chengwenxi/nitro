...
type TransactionStreamer struct {
	...
	broadcasterQueuedMessages    []mtstate.MessageWithMetadata
	broadcasterQueuedMessagesPos uint64

	latestBlockAndMessageMutex sync.Mutex
	latestBlock                *types.Block
	latestMessage              *mtos.L1IncomingMessage
...
func (s *TransactionStreamer) ReorgTo(count mtutil.MessageIndex) error {
	return s.ReorgToAndEndBatch(s.db.NewBatch(), count)
}

func (s *TransactionStreamer) ReorgToAndEndBatch(batch ethdb.Batch, count mtutil.MessageIndex) error {
...
func (s *TransactionStreamer) reorgToInternal(batch ethdb.Batch, count mtutil.MessageIndex) error {
	if count == 0 {
...
func (s *TransactionStreamer) GetMessage(seqNum mtutil.MessageIndex) (mtstate.MessageWithMetadata, error) {
	key := dbKey(messagePrefix, uint64(seqNum))
	data, err := s.db.Get(key)
	if err != nil {
		return mtstate.MessageWithMetadata{}, err
	}
	var message mtstate.MessageWithMetadata
	err = rlp.DecodeBytes(data, &message)
	return message, err
}

// Note: if changed to acquire the mutex, some internal users may need to be updated to a non-locking version.
func (s *TransactionStreamer) GetMessageCount() (mtutil.MessageIndex, error) {
	...
	return mtutil.MessageIndex(pos), nil
}

func (s *TransactionStreamer) AddMessages(pos mtutil.MessageIndex, force bool, messages []mtstate.MessageWithMetadata) error {
	return s.AddMessagesAndEndBatch(pos, force, messages, nil)
}

func (s *TransactionStreamer) AddBroadcastMessages(pos mtutil.MessageIndex, messages []mtstate.MessageWithMetadata) error {
	for i, message := range messages {
		msgPos := pos + mtutil.MessageIndex(i)
	...
	if currentMessageCount >= pos {
		...
		broadcasterQueuedMessagesPos := mtutil.MessageIndex(atomic.LoadUint64(&s.broadcasterQueuedMessagesPos))
		if len(s.broadcasterQueuedMessages) > 0 && broadcasterQueuedMessagesPos+mtutil.MessageIndex(len(s.broadcasterQueuedMessages)) == pos {
			...
			if len(s.broadcasterQueuedMessages) > 0 {
				log.Warn(
					...
					"expectedNextPos", broadcasterQueuedMessagesPos+mtutil.MessageIndex(len(s.broadcasterQueuedMessages)),
					"gotPos", pos,
				)
			}
...
func (s *TransactionStreamer) AddFakeInitMessage() error {
	return s.AddMessages(0, false, []mtstate.MessageWithMetadata{{
		Message: &mtos.L1IncomingMessage{
			Header: &mtos.L1IncomingMessageHeader{
				Kind:      mtos.L1MessageType_Initialize,
...
func (s *TransactionStreamer) GetMessageCountSync() (mtutil.MessageIndex, error) {
	s.insertionMutex.Lock()
	defer s.insertionMutex.Unlock()
	return s.GetMessageCount()
}

func (s *TransactionStreamer) AddMessagesAndEndBatch(pos mtutil.MessageIndex, force bool, messages []mtstate.MessageWithMetadata, batch ethdb.Batch) error {
	s.insertionMutex.Lock()
	defer s.insertionMutex.Unlock()
...
func (s *TransactionStreamer) addMessagesAndEndBatchImpl(pos mtutil.MessageIndex, force bool, messages []mtstate.MessageWithMetadata, batch ethdb.Batch) error {
	...
	afterCount := pos + mtutil.MessageIndex(len(messages))
	broadcasterQueuedMessagesPos := mtutil.MessageIndex(atomic.LoadUint64(&s.broadcasterQueuedMessagesPos))
	...
	for {
		...
		if bytes.Equal(haveMessage, wantMessage) {
			...
			var dbMessageParsed mtstate.MessageWithMetadata
			...
			if err != nil {
				...
				var gotHeader *mtos.L1IncomingMessageHeader
...
func messageFromTxes(header *mtos.L1IncomingMessageHeader, txes types.Transactions, txErrors []error) (*mtos.L1IncomingMessage, error) {
	...
	if len(txes) == 1 && txErrors[0] == nil {
		...
		l2Message = append(l2Message, mtos.L2MessageKind_SignedTx)
		l2Message = append(l2Message, txBytes...)
	} else {
		l2Message = append(l2Message, mtos.L2MessageKind_Batch)
		sizeBuf := make([]byte, 8)
		for i, tx := range txes {
			if txErrors[i] != nil {
				continue
	...
	return &mtos.L1IncomingMessage{
		Header: header,
		L2msg:  l2Message,
	}, nil
}

func (s *TransactionStreamer) SequenceTransactions(header *mtos.L1IncomingMessageHeader, txes types.Transactions, hooks *mtos.SequencingHooks) error {
	s.insertionMutex.Lock()
	defer s.insertionMutex.Unlock()
	...
	startTime := time.Now()
	block, receipts, err := mtos.ProduceBlockAdvanced(
		header,
		txes,
		delayedMessagesRead,
		lastBlockHeader,
	...
	msgWithMeta := mtstate.MessageWithMetadata{
		Message:             msg,
	...
	if err := s.writeMessages(pos, []mtstate.MessageWithMetadata{msgWithMeta}, nil); err != nil {
		return err
	}
...
func (s *TransactionStreamer) SequenceDelayedMessages(ctx context.Context, messages []*mtos.L1IncomingMessage, firstDelayedSeqNum uint64) error {
	s.insertionMutex.Lock()
	defer s.insertionMutex.Unlock()
	...
	messagesWithMeta := make([]mtstate.MessageWithMetadata, 0, len(messages))
	for i, message := range messages {
		newMessage := mtstate.MessageWithMetadata{
			Message:             message,
			DelayedMessagesRead: delayedMessagesRead + uint64(i) + 1,
		}
		messagesWithMeta = append(messagesWithMeta, newMessage)
		if s.coordinator != nil {
			if err := s.coordinator.SequencingMessage(pos+mtutil.MessageIndex(i), &newMessage); err != nil {
				return err
			}
		}
	}
	...
	for i, msg := range messagesWithMeta {
		if s.broadcastServer != nil {
			s.broadcastServer.BroadcastSingle(msg, pos+mtutil.MessageIndex(i))
		}
	}
...
func (s *TransactionStreamer) GetGenesisBlockNumber() (uint64, error) {
	return s.bc.Config().MantleChainParams.GenesisBlockNum, nil
}

func (s *TransactionStreamer) BlockNumberToMessageCount(blockNum uint64) (mtutil.MessageIndex, error) {
	genesis, err := s.GetGenesisBlockNumber()
	if err != nil {
		return 0, err
	}
	return mtutil.BlockNumberToMessageCount(blockNum, genesis), nil
}

func (s *TransactionStreamer) MessageCountToBlockNumber(messageNum mtutil.MessageIndex) (int64, error) {
	genesis, err := s.GetGenesisBlockNumber()
	if err != nil {
		return 0, err
	}
	return mtutil.MessageCountToBlockNumber(messageNum, genesis), nil
}
...
func (s *TransactionStreamer) writeMessages(pos mtutil.MessageIndex, messages []mtstate.MessageWithMetadata, batch ethdb.Batch) error {
	if batch == nil {
		batch = s.db.NewBatch()
	}
	for i, msg := range messages {
...
func (s *TransactionStreamer) createBlocks(ctx context.Context) error {
	...
	for pos < msgCount {
		...
		startTime := time.Now()
		block, receipts, err := mtos.ProduceBlock(
			msg.Message,
