...
func (s *TransactionStreamer) AddBroadcastMessages(feedMessages []*broadcaster.BroadcastFeedMessage) error {
	...
	if currentMessageCount >= startingSeqNum {
		...
		if len(s.broadcasterQueuedMessages) > 0 && broadcasterQueuedMessagesPos+mtutil.MessageIndex(len(s.broadcasterQueuedMessages)) == startingSeqNum {
...
func (s *TransactionStreamer) SequenceDelayedMessages(ctx context.Context, messages []*mtos.L1IncomingMessage, firstDelayedSeqNum uint64) error {
	...
	for i, msg := range messagesWithMeta {
		if s.broadcastServer != nil {
			if err := s.broadcastServer.BroadcastSingle(msg, pos+mtutil.MessageIndex(i)); err != nil {
				return err
			}
		}
	}
