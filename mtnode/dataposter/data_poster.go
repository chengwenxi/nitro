...
const minRbfIncrease mtmath.Bips = mtmath.OneInBips * 11 / 10
...
func (p *DataPoster[Meta]) getFeeAndTipCaps(ctx context.Context, lastTipCap *big.Int, dataCreatedAt time.Time) (*big.Int, *big.Int, error) {
	...
	if lastTipCap != nil {
		newTipCap = mtmath.BigMax(newTipCap, mtmath.BigMulByBips(lastTipCap, minRbfIncrease))
	}
	...
	if mtmath.BigGreaterThan(newFeeCap, maxFeeCap) {
		logLevel := log.Info
...
func (p *DataPoster[Meta]) replaceTx(ctx context.Context, prevTx *queuedTransaction[Meta]) error {
	...
	newFeeCap = mtmath.BigMin(newFeeCap, maxFeeCap)
	minNewFeeCap := mtmath.BigMulByBips(prevTx.Data.GasFeeCap, minRbfIncrease)
	newTx := *prevTx
...
func (p *DataPoster[Meta]) updateState(ctx context.Context) error {
	...
	p.lastBlock = mtmath.BigSub(header.Number, new(big.Int).SetUint64(p.config().L1LookBehind))
