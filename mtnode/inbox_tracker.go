...
func NewInboxTracker(db ethdb.Database, txStreamer *TransactionStreamer, das mtstate.DataAvailabilityReader) (*InboxTracker, error) {
	if txStreamer.bc.Config().MantleChainParams.DataAvailabilityCommittee && das == nil {
...
type BatchMetadata struct {
	...
	MessageCount        mtutil.MessageIndex
...
func (t *InboxTracker) GetBatchMessageCount(seqNum uint64) (mtutil.MessageIndex, error) {
...
func (t *InboxTracker) GetDelayedMessageAndAccumulator(seqNum uint64) (*mtos.L1IncomingMessage, common.Hash, error) {
	data, acc, err := t.getDelayedMessageBytesAndAccumulator(seqNum)
	if err != nil {
		return nil, acc, err
	}
	message, err := mtos.ParseIncomingL1Message(bytes.NewReader(data))
	return message, acc, err
}

func (t *InboxTracker) GetDelayedMessage(seqNum uint64) (*mtos.L1IncomingMessage, error) {
...
func (t *InboxTracker) setDelayedCountReorgAndWriteBatch(batch ethdb.Batch, newDelayedCount uint64, canReorgBatches bool) error {
	...
	if reorgSeqBatchesToCount != nil {
		...
		var prevMesssageCount mtutil.MessageIndex
		if count > 0 {
...
func (t *InboxTracker) AddSequencerBatches(ctx context.Context, client mtutil.L1Interface, batches []*SequencerInboxBatch) error {
	if len(batches) == 0 {
		return nil
	}
	t.mutex.Lock()
	...
	multiplexer := mtstate.NewInboxMultiplexer(backend, prevbatchmeta.DelayedMessageCount, t.das, mtstate.KeysetValidate)
	batchMessageCounts := make(map[uint64]mtutil.MessageIndex)
	...
	newMessageCount := prevbatchmeta.MessageCount + mtutil.MessageIndex(len(messages))
