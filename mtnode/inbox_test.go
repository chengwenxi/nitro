...
type blockTestState struct {
	...
	numMessages mtutil.MessageIndex
	blockNumber uint64
}
	...
	for i := 1; i < 100; i++ {
		if i%10 == 0 {
			...
			for j := 0; j < numMessages; j++ {
				...
				var l2Message []byte
				l2Message = append(l2Message, mtos.L2MessageKind_ContractTx)
				...
				messages = append(messages, mtstate.MessageWithMetadata{
			...
			state.numMessages += mtutil.MessageIndex(len(messages))
