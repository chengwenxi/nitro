...
type Sequencer struct {
	...
	txRetryQueue    mtutil.Queue[txQueueItem]
...
func (s *Sequencer) PublishTransaction(ctx context.Context, tx *types.Transaction) error {
	...
	if tx.Type() >= types.MantleDepositTxType {
		// Should be unreachable due to UnmarshalBinary not accepting Mantle internal txs
...
func (s *Sequencer) preTxFilter(_ *params.ChainConfig, _ *types.Header, statedb *state.StateDB, _ *mtosState.MtosState, _ *types.Transaction) error {
	return nil
}

func (s *Sequencer) postTxFilter(_ *mtosState.MtosState, _ *types.Transaction, _ common.Address, dataGas uint64, result *core.ExecutionResult) error {
	if result.Err != nil && result.UsedGas > dataGas && result.UsedGas-dataGas <= s.config().MaxRevertGasReject {
		return mantle.NewRevertReason(result)
	}
	return nil
}
...
func (s *Sequencer) createBlock(ctx context.Context) (returnValue bool) {
	...
	header := &mtos.L1IncomingMessageHeader{
		Kind:        mtos.L1MessageType_L2Message,
	...
	for i, err := range hooks.TxErrors {
		...
		if errors.Is(err, core.ErrGasLimitReached) {
			...
			if madeBlock && !errors.Is(err, mtos.ErrMaxGasLimitReached) {
