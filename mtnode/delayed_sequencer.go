...
func (d *DelayedSequencer) update(ctx context.Context, lastBlockHeader *types.Header) error {
	...
	finalized := mtmath.BigSub(lastBlockHeader.Number, big.NewInt(d.config.FinalizeDistance))
	if finalized.Sign() < 0 {
	...
	if d.waitingForFinalizedBlock != nil && mtmath.BigLessThan(finalized, d.waitingForFinalizedBlock) {
		return nil
	}
	...
	d.waitingForFinalizedBlock = mtmath.BigAddByUint(lastBlockHeader.Number, 1)
	...
	var messages []*mtos.L1IncomingMessage
	for pos < dbDelayedCount {
		msg, acc, err := d.inbox.GetDelayedMessageAndAccumulator(pos)
		if err != nil {
			return err
		}
		blockNumber := mtmath.UintToBig(msg.Header.BlockNumber)
