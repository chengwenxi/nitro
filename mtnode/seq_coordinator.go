...
func (c *SeqCoordinator) getRemoteMsgCountImpl(ctx context.Context, r redis.Cmdable) (mtutil.MessageIndex, error) {
	...
	for msgToRead < readUntil {
		...
		if err != nil {
			...
			message = mtstate.MessageWithMetadata{
				Message:             mtstate.InvalidL1Message,
...
func (c *SeqCoordinator) SequencingMessage(pos mtutil.MessageIndex, msg *mtstate.MessageWithMetadata) error {
	if !c.CurrentlyChosen() {
