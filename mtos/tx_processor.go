...
var mtosAddress = types.MtosAddress

const GasEstimationL1PricePadding mtmath.Bips = 11000 // pad estimates by 10%

// A TxProcessor is created and freed for every L2 transaction.
// It tracks state for MtOS, allowing it infuence in Geth's tx processing.
...
func NewTxProcessor(evm *vm.EVM, msg core.Message) *TxProcessor {
	tracingInfo := util.NewTracingInfo(evm, msg.From(), mtosAddress, util.TracingBeforeEVM)
	mtosState := mtosState.OpenSystemMtosStateOrPanic(evm.StateDB, tracingInfo, false)
	return &TxProcessor{
		msg:                 msg,
		state:               mtosState,
...
func takeFunds(pool *big.Int, take *big.Int) *big.Int {
	...
	if mtmath.BigLessThan(pool, take) {
...
func (p *TxProcessor) StartTxHook() (endTxNow bool, gasUsed uint64, err error, returnData []byte) {
	...
	startTracer := func() func() {
		...
		p.state = mtosState.OpenSystemMtosStateOrPanic(evm.StateDB, tracingInfo, false)
	...
	case *types.MantleSubmitRetryableTx:
		...
		if mtmath.BigLessThan(tx.MaxSubmissionFee, submissionFee) {
		...
		submissionFeeRefund := takeFunds(availableRefund, mtmath.BigSub(tx.MaxSubmissionFee, submissionFee))
		...
		maxGasCost := mtmath.BigMulByUint(tx.GasFeeCap, usergas)
		maxFeePerGasTooLow := mtmath.BigLessThan(tx.GasFeeCap, basefee)
		...
		if mtmath.BigLessThan(balance, maxGasCost) || usergas < params.TxGas || maxFeePerGasTooLow {
		...
		gascost := mtmath.BigMulByUint(basefee, usergas)
		...
		gasPriceRefund := mtmath.BigMulByUint(mtmath.BigSub(tx.GasFeeCap, basefee), tx.Gas)
		...
		if evm.Config.Debug {
			redeem, err := util.PackMtRetryableTxRedeem(ticketId)
			if err == nil {
				tracingInfo.MockCall(redeem, usergas, from, types.MtRetryableTxAddress, common.Big0)
			} else {
	...
	case *types.MantleRetryTx:
		...
		prepaid := mtmath.BigMulByUint(evm.Context.BaseFee, tx.Gas)
...
func (p *TxProcessor) GasChargingHook(gasRemaining *uint64) error {
	...
	if p.msg.RunMode() == types.MessageGasEstimationMode {
		...
		adjustedPrice := mtmath.BigMulByFrac(gasPrice, 7, 8) // assume congestion
		if mtmath.BigLessThan(adjustedPrice, minGasPrice) {
			adjustedPrice = minGasPrice
		}
		gasPrice = adjustedPrice

		// Pad the L1 cost in case the L1 gas price rises
		posterCost = mtmath.BigMulByBips(posterCost, GasEstimationL1PricePadding)
	}

	if gasPrice.Sign() > 0 {
		posterCostInL2Gas := mtmath.BigDiv(posterCost, gasPrice) // the cost as if it were an amount of gas
		if !posterCostInL2Gas.IsUint64() {
			posterCostInL2Gas = mtmath.UintToBig(math.MaxUint64)
		}
		p.posterGas = posterCostInL2Gas.Uint64()
		p.PosterFee = mtmath.BigMul(posterCostInL2Gas, gasPrice) // round down
...
func (p *TxProcessor) EndTxHook(gasLeft uint64, success bool) {
	...
	if underlyingTx != nil && underlyingTx.Type() == types.MantleRetryTxType {
		inner, _ := underlyingTx.GetInner().(*types.MantleRetryTx)

		// undo Geth's refund to the From address
		gasRefund := mtmath.BigMulByUint(gasPrice, gasLeft)
		...
		refundNetworkFee := func(amount *big.Int) {
			...
			err = util.TransferBalance(&networkFeeAccount, &inner.From, mtmath.BigSub(amount, toRefundAddr), p.evm, scenario, "refund")
			if err != nil {
				log.Error(errLog, "err", err)
			}
		}
		...
		takeFunds(maxRefund, mtmath.BigMulByUint(gasPrice, gasUsed))
		// Refund any unused gas, without overdrafting the L1 deposit.
		refundNetworkFee(gasRefund)

		if success {
			// we don't want to charge for this
			tracingInfo := util.NewTracingInfo(p.evm, mtosAddress, p.msg.From(), scenario)
			state := mtosState.OpenSystemMtosStateOrPanic(p.evm.StateDB, tracingInfo, false)
		...
		p.state.Restrict(p.state.L2PricingState().AddToGasPool(-mtmath.SaturatingCast(gasUsed)))
		return
	}

	totalCost := mtmath.BigMul(gasPrice, mtmath.UintToBig(gasUsed)) // total cost = price of gas * gas burnt
	computeCost := mtmath.BigSub(totalCost, p.PosterFee)            // total cost = network's compute + poster's L1 costs
	...
	if p.state.FormatVersion() > 4 {
		...
		if infraFeeAccount != (common.Address{}) {
			...
			p.state.Restrict(err)
			if mtmath.BigLessThan(gasPrice, infraFee) {
				infraFee = gasPrice
			}
			computeGas := mtmath.SaturatingUSub(gasUsed, p.posterGas)
			infraComputeCost := mtmath.BigMulByUint(infraFee, computeGas)
			util.MintBalance(&infraFeeAccount, infraComputeCost, p.evm, scenario, purpose)
			computeCost = mtmath.BigSub(computeCost, infraComputeCost)
		}
	}
	if mtmath.BigGreaterThan(computeCost, common.Big0) {
	...
	if p.msg.GasPrice().Sign() > 0 { // in tests, gas price could be 0
		// MtOS's gas pool is meant to enforce the computational speed-limit.
		...
		p.state.Restrict(p.state.L2PricingState().AddToGasPool(-mtmath.SaturatingCast(computeGas)))
	}
}
...
func (p *TxProcessor) ScheduledTxes() types.Transactions {
	...
	for _, log := range logs {
		if log.Address != MtRetryableTxAddress || log.Topics[0] != RedeemScheduledEventID {
			continue
		}
		event := &precompilesgen.MtRetryableTxRedeemScheduled{}
...
func (p *TxProcessor) L1BlockNumber(blockCtx vm.BlockContext) (uint64, error) {
	...
	tracingInfo := util.NewTracingInfo(p.evm, p.msg.From(), mtosAddress, util.TracingDuringEVM)
	state, err := mtosState.OpenSystemMtosState(p.evm.StateDB, tracingInfo, false)
	if err != nil {
		return 0, err
	}
