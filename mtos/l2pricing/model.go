...
var InitialGasPoolTargetBips = mtmath.PercentToBips(80)
var InitialGasPoolWeightBips = mtmath.PercentToBips(60)
...
func (ps *L2PricingState) AddToGasPool(gas int64) error {
	...
	backlog = mtmath.SaturatingUCast(mtmath.SaturatingSub(int64(backlog), gas))
...
func (ps *L2PricingState) UpdatePricingModel(l2BaseFee *big.Int, timePassed uint64, debug bool) {
	...
	if backlog > tolerance*speedLimit {
		...
		exponentBips := mtmath.NaturalToBips(excess) / mtmath.Bips(inertia*speedLimit)
		baseFee = mtmath.BigMulByBips(minBaseFee, mtmath.ApproxExpBasisPoints(exponentBips))
	}
