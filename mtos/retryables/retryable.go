...
func (rs *RetryableState) RetryableSizeBytes(id common.Hash, currentTime uint64) (uint64, error) {
	...
	calldata := 32 + 32*mtmath.WordsForBytes(size) // length + contents
...
func (retryable *Retryable) MakeTx(chainId *big.Int, nonce uint64, gasFeeCap *big.Int, gas uint64, ticketId common.Hash, refundTo common.Address, maxRefund *big.Int, submissionFeeRefund *big.Int) (*types.MantleRetryTx, error) {
...
func RetryableSubmissionFee(calldataLengthInBytes int, l1BaseFee *big.Int) *big.Int {
	return mtmath.BigMulByUint(l1BaseFee, uint64(1400+6*calldataLengthInBytes))
}
