...
func (info *TracingInfo) RecordStorageGet(key common.Hash) {
	...
	if info.Scenario == TracingDuringEVM {
		...
		tracer.CaptureMantleStorageGet(key, info.Depth, info.Scenario == TracingBeforeEVM)
	}
}
...
func (info *TracingInfo) RecordStorageSet(key, value common.Hash) {
	...
	if info.Scenario == TracingDuringEVM {
		...
		tracer.CaptureMantleStorageSet(key, value, info.Depth, info.Scenario == TracingBeforeEVM)
	}
}
