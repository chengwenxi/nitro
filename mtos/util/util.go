...
var PackMtRetryableTxRedeem func(...interface{}) ([]byte, error)
...
func init() {
	...
	InverseAddressAliasOffset = mtmath.BigSub(new(big.Int).Lsh(big.NewInt(1), 160), AddressAliasOffset)
	...
	ParseRedeemScheduledLog = logParser(precompilesgen.MtRetryableTxABI, "RedeemScheduled")
	ParseL2ToL1TxLog = logParser(precompilesgen.MtSysABI, "L2ToL1Tx")
	ParseL2ToL1TransactionLog = logParser(precompilesgen.MtSysABI, "L2ToL1Transaction")

	acts := precompilesgen.MtosActsABI
	...
	PackMtRetryableTxRedeem, _ = callParser(precompilesgen.MtRetryableTxABI, "redeem")
}
...
func DoesTxTypeAlias(txType *byte) bool {
	...
	switch *txType {
	case types.MantleUnsignedTxType:
		fallthrough
	case types.MantleContractTxType:
		fallthrough
	case types.MantleRetryTxType:
		return true
	}
	return false
}
...
func TxTypeHasPosterCosts(txType byte) bool {
	...
	case types.MantleRetryTxType:
		fallthrough
	case types.MantleInternalTxType:
		fallthrough
	case types.MantleSubmitRetryableTxType:
		return false
	}
	return true
}
