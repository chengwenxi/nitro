...
type L1PricingState struct {
	...
	lastSurplus          storage.StorageBackedBigInt // introduced in MtOS version 2
	perBatchGasCost      storage.StorageBackedInt64  // introduced in MtOS version 3
	amortizedCostCapBips storage.StorageBackedUint64 // in basis points; introduced in MtOS version 3
}
...
var InitialEquilibrationUnitsV0 = mtmath.UintToBig(60 * params.TxDataNonZeroGasEIP2028 * 100000)
var InitialEquilibrationUnitsV6 = mtmath.UintToBig(params.TxDataNonZeroGasEIP2028 * 10000000)
...
func (ps *L1PricingState) SetLastSurplus(val *big.Int, mtosVersion uint64) error {
	if mtosVersion < 7 {
...
func (ps *L1PricingState) UpdateForBatchPosterSpending(
	...
	if unitsAllocated > 0 {
		...
		if err := ps.SetLastSurplus(surplus, mtosVersion); err != nil {
			return err
		}
...
var randV = mtmath.BigMulByUint(params.MantleOneChainConfig().ChainID, 3)
...
func (ps *L1PricingState) PosterDataCost(message core.Message, poster common.Address) (*big.Int, uint64) {
	...
	units := ps.getPosterUnitsWithoutCache(tx, poster)
	units = mtmath.UintMulByBips(units+estimationPaddingUnits, mtmath.OneInBips+estimationPaddingBasisPoints)
	pricePerUnit, _ := ps.PricePerUnit()
	return am.BigMulByUint(pricePerUnit, units), units
}

func byteCountAfterBrotli0(input []byte) (uint64, error) {
	compressed, err := mtcompress.CompressFast(input)
	if err != nil {
		return 0, err
	}
