...
var MtRetryableTxAddress common.Address
var MtSysAddress common.Address
...
func createNewHeader(prevHeader *types.Header, l1info *L1Info, state *mtosState.MtosState, chainConfig *params.ChainConfig) *types.Header {
...
type SequencingHooks struct {
	...
	PreTxFilter            func(*params.ChainConfig, *types.Header, *state.StateDB, *mtosState.MtosState, *types.Transaction) error
	PostTxFilter           func(*mtosState.MtosState, *types.Transaction, common.Address, uint64, *core.ExecutionResult) error
}
...
func noopSequencingHooks() *SequencingHooks {
	return &SequencingHooks{
		[]error{},
		false,
		func(*params.ChainConfig, *types.Header, *state.StateDB, *mtosState.MtosState, *types.Transaction) error {
			return nil
		},
		func(*mtosState.MtosState, *types.Transaction, common.Address, uint64, *core.ExecutionResult) error {
			return nil
		},
	}
}
...
func ProduceBlockAdvanced(
	...
	for len(txes) > 0 || len(redeems) > 0 {
		...
		if len(redeems) > 0 {
			...
			retry, ok := (tx.GetInner()).(*types.MantleRetryTx)
			if !ok {
		...
		receipt, scheduled, err := (func() (*types.Receipt, types.Transactions, error) {
			...
			if gasPrice.Sign() > 0 {
				...
				posterCostInL2Gas := mtmath.BigDiv(posterCost, gasPrice)
		...
		for _, txLog := range receipt.Logs {
			if txLog.Address == MtSysAddress {
				...
				case L2ToL1TransactionEventID:
					event := &precompilesgen.MtSysL2ToL1Transaction{}
				...
				case L2ToL1TxEventID:
					event := &precompilesgen.MtSysL2ToL1Tx{}
	...
	if !mtmath.BigEquals(balanceDelta, expectedBalanceDelta) {
...
func FinalizeBlock(header *types.Header, txs types.Transactions, statedb *state.StateDB) {
	if header != nil {
		state, _ := mtosState.OpenSystemMtosState(statedb, nil, true)
		...
		mantleHeader := types.HeaderInfo{
			SendRoot:          root,
			SendCount:         size,
			L1BlockNumber:     nextL1BlockNumber,
			MtOSFormatVersion: state.FormatVersion(),
		}
		mantleHeader.UpdateHeaderWithInfo(header)
	}
}
