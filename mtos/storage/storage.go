...
// Storage allows MtOS to store data persistently in the Ethereum-compatible stateDB. This is represented in
...
func NewGeth(statedb vm.StateDB, burner burn.Burner) *Storage {
	...
	statedb.SetNonce(account, 1) // setting the nonce ensures Geth won't treat MtOS as empty
	return &Storage{
		account:    account,
		db:         statedb,
		storageKey: []byte{},
...
func (sto *Storage) Keccak(data ...[]byte) ([]byte, error) {
	...
	cost := 30 + 6*mtmath.WordsForBytes(uint64(byteCount))
...
func (sbu *StorageBackedBips) Get() (mtmath.Bips, error) {
	value, err := sbu.backing.Get()
	return mtmath.Bips(value), err
}

func (sbu *StorageBackedBips) Set(bips mtmath.Bips) error {
