...
func MakeGenesisBlock(parentHash common.Hash, blockNumber uint64, timestamp uint64, stateRoot common.Hash, chainConfig *params.ChainConfig) *types.Block {
	...
	genesisHeaderInfo := types.HeaderInfo{
		...
		SendCount:         0,
		L1BlockNumber:     0,
		MtOSFormatVersion: chainConfig.MantleChainParams.InitialMtOSVersion,
	}
...
func InitializeMtosInDatabase(db ethdb.Database, initData statetransfer.InitDataReader, chainConfig *params.ChainConfig, timestamp uint64, accountsPerSync uint) (common.Hash, error) {
	...
	mtosState, err := InitializeMtosState(statedb, burner, chainConfig)
	if err != nil {
		log.Fatal("failed to open the MtOS state", err)
	}

	addrTable := mtosState.AddressTable()
	...
	err = initializeRetryables(statedb, mtosState.RetryableState(), retryableReader, timestamp)
	if err != nil {
		return common.Hash{}, err
	}
	...
	for accountDataReader.More() {
		...
		err = initializeMtosAccount(statedb, mtosState, *account)
		if err != nil {
			return common.Hash{}, err
		}
...
func initializeRetryables(statedb *state.StateDB, rs *retryables.RetryableState, initData statetransfer.RetryableDataReader, currentTimestamp uint64) error {
	...
	sort.Slice(retryablesList, func(i, j int) bool {
		...
		if a.Timeout == b.Timeout {
			return mtmath.BigLessThan(a.Id.Big(), b.Id.Big())
		}
...
func initializeMtosAccount(statedb *state.StateDB, mtosState *MtosState, account statetransfer.AccountInitializationInfo) error {
	l1pState := mtosState.L1PricingState()
