...
func TestJsonMarshalUnmarshal(t *testing.T) {
	...
	tryMarshalUnmarshal(
		&statetransfer.MtosInitializationInfo{
...
func tryMarshalUnmarshal(input *statetransfer.MtosInitializationInfo, t *testing.T) {
	...
	output := statetransfer.MtosInitializationInfo{}
	...
	var initData statetransfer.MtosInitializationInfo
	...
	stateroot, err := InitializeMtosInDatabase(raw, initReader, params.MantleDevTestChainConfig(), 0, 0)
	Require(t, err)

	stateDb, err := state.New(stateroot, state.NewDatabase(raw), nil)
	Require(t, err)

	mtState, err := OpenMtosState(stateDb, &burn.SystemBurner{})
	Require(t, err)
	checkAddressTable(mtState, input.AddressTableContents, t)
	checkRetryables(mtState, input.RetryableData, t)
	checkAccounts(stateDb, mtState, input.Accounts, t)
}
...
func checkAddressTable(mtState *MtosState, addrTable []common.Address, t *testing.T) {
	atab := mtState.AddressTable()
...
func checkRetryables(mtState *MtosState, expected []statetransfer.InitializationDataForRetryable, t *testing.T) {
	ret := mtState.RetryableState()
...
func checkAccounts(db *state.StateDB, mtState *MtosState, accts []statetransfer.AccountInitializationInfo, t *testing.T) {
	l1p := mtState.L1PricingState()
