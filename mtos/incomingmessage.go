...
func (msg *L1IncomingMessage) ParseL2Transactions(chainId *big.Int, batchFetcher InfallibleBatchFetcher) (types.Transactions, error) {
	...
	case L1MessageType_L2FundedByL1:
		...
		deposit := types.NewTx(&types.MantleDepositTx{
			ChainId:     chainId,
...
func parseL2Message(rd io.Reader, poster common.Address, timestamp uint64, requestId *common.Hash, chainId *big.Int, depth int) (types.Transactions, error) {
	...
	case L2MessageKind_SignedTx:
		...
		if newTx.Type() >= types.MantleDepositTxType {
...
func parseSubmitRetryableMessage(rd io.Reader, header *L1IncomingMessageHeader, chainId *big.Int) (*types.Transaction, error) {
	...
	tx := &types.MantleSubmitRetryableTx{
		ChainId:          chainId,
...
func parseBatchPostingReportMessage(rd io.Reader, chainId *big.Int, batchFetcher InfallibleBatchFetcher) (*types.Transaction, error) {
	...
	keccakWords := mtmath.WordsForBytes(uint64(len(batchData)))
	...
	return types.NewTx(&types.MantleInternalTx{
		ChainId: chainId,
		Data:    data,
