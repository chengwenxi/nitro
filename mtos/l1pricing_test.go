...
func expectedResultsForL1Test(input *l1PricingTest) *l1TestExpectedResults {
	...
	availableFunds := mtmath.UintToBig(3 * input.fundsCollectedPerSecond)
	uncappedAvailableFunds := availableFunds
	if input.amortizationCapBips != 0 {
		availableFundsCap := mtmath.BigMulByBips(mtmath.BigMulByUint(
			mtmath.UintToBig(input.unitsPerSecond),
			input.l1BasefeeGwei*params.GWei),
			mtmath.SaturatingCastToBips(input.amortizationCapBips),
		)
		if mtmath.BigLessThan(availableFundsCap, availableFunds) {
			availableFunds = availableFundsCap
		}
	}
	fundsWantedForRewards := big.NewInt(int64(input.unitReward * input.unitsPerSecond))
	unitsAllocated := mtmath.UintToBig(input.unitsPerSecond)
	if mtmath.BigLessThan(availableFunds, fundsWantedForRewards) {
		ret.rewardRecipientBalance = availableFunds
	} else {
		ret.rewardRecipientBalance = fundsWantedForRewards
	}
	availableFunds = mtmath.BigSub(availableFunds, ret.rewardRecipientBalance)
	uncappedAvailableFunds = mtmath.BigSub(uncappedAvailableFunds, ret.rewardRecipientBalance)
	ret.unitsRemaining = (3 * input.unitsPerSecond) - unitsAllocated.Uint64()

	maxCollectable := big.NewInt(int64(input.fundsSpent))
	if mtmath.BigLessThan(availableFunds, maxCollectable) {
		maxCollectable = availableFunds
	}
	ret.fundsReceived = maxCollectable
	uncappedAvailableFunds = mtmath.BigSub(uncappedAvailableFunds, maxCollectable)
...
func _testL1PricingFundsDue(t *testing.T, testParams *l1PricingTest, expectedResults *l1TestExpectedResults) {
	...
	mtosSt, err := mtosState.OpenMtosState(evm.StateDB, burner)
	Require(t, err)

	l1p := mtosSt.L1PricingState()
	...
	Require(t, err)
	version := mtosSt.FormatVersion()
	scenario := util.TracingDuringEVM
	err = l1p.UpdateForBatchPosterSpending(
		evm.StateDB, evm, version, 1, 3, firstPoster, mtmath.UintToBig(testParams.fundsSpent), mtmath.UintToBig(testParams.l1BasefeeGwei*params.GWei), scenario,
	)
	Require(t, err)
	rewardRecipientBalance := evm.StateDB.GetBalance(rewardAddress)
	if !mtmath.BigEquals(rewardRecipientBalance, expectedResults.rewardRecipientBalance) {
	...
	if !mtmath.BigEquals(fundsReceived, expectedResults.fundsReceived) {
	...
	if !mtmath.BigEquals(fundsStillHeld, expectedResults.fundsStillHeld) {
...
func TestUpdateTimeUpgradeBehavior(t *testing.T) {
	...
	l1p := mtosSt.L1PricingState()
	amount := mtmath.UintToBig(10 * params.GWei)
...
func _testL1PriceEquilibration(t *testing.T, initialL1BasefeeEstimate *big.Int, equilibriumL1BasefeeEstimate *big.Int) {
	evm := newMockEVMForTesting()
	stateDb := evm.StateDB
	state, err := mtosState.OpenMtosState(stateDb, burn.NewSystemBurner(nil, false))
	Require(t, err)
	...
	for i := 0; i < 10; i++ {
		...
		Require(t, err)
		feesToAdd := mtmath.BigMulByUint(currentPricePerUnit, unitsToAdd)
		...
		err = l1p.UpdateForBatchPosterSpending(
			...
			uint64(10*(i+1)+5),
			bpAddr,
			mtmath.BigMulByUint(equilibriumL1BasefeeEstimate, unitsToAdd),
			equilibriumL1BasefeeEstimate,
			util.TracingBeforeEVM,
		)
		Require(t, err)
	}
	expectedMovement := mtmath.BigSub(equilibriumL1BasefeeEstimate, initialL1BasefeeEstimate)
	actualPricePerUnit, err := l1p.PricePerUnit()
	Require(t, err)
	actualMovement := mtmath.BigSub(actualPricePerUnit, initialL1BasefeeEstimate)
...
func _withinOnePercent(v1, v2 *big.Int) bool {
	if mtmath.BigMulByUint(v1, 100).Cmp(mtmath.BigMulByUint(v2, 101)) > 0 {
		return false
	}
	if mtmath.BigMulByUint(v2, 100).Cmp(mtmath.BigMulByUint(v1, 101)) > 0 {
		return false
	}
	return true
}
...
func newMockEVMForTesting() *vm.EVM {
	...
	_, statedb := mtosState.NewMtosMemoryBackedMtOSState()
