...
use crate::{
    ...
    value::{FunctionType, MtValueType},
...
pub fn get_host_impl(module: &str, name: &str) -> eyre::Result<Function> {
    ...
    match (module, name) {
        ("env", "wavm_caller_load8") => {
            ty = FunctionType::new(vec![MtValueType::I32], vec![MtValueType::I32]);
            opcode!(LocalGet, 0);
        ...
        ("env", "wavm_caller_store8") => {
            ty = FunctionType::new(vec![MtValueType::I32; 2], vec![]);
            opcode!(LocalGet, 0);
            opcode!(LocalGet, 1);
        ...
        ("env", "wavm_get_globalstate_u64") => {
            ty = FunctionType::new(vec![MtValueType::I32], vec![MtValueType::I64]);
            opcode!(LocalGet, 0);
            opcode!(GetGlobalStateU64);
        }
        ("env", "wavm_set_globalstate_u64") => {
            ty = FunctionType::new(vec![MtValueType::I32, MtValueType::I64], vec![]);
            opcode!(LocalGet, 0);
            opcode!(LocalGet, 1);
            opcode!(SetGlobalStateU64);
        }
        ("env", "wavm_read_pre_image") => {
            ty = FunctionType::new(vec![MtValueType::I32; 2], vec![MtValueType::I32]);
            opcode!(LocalGet, 0);
            opcode!(LocalGet, 1);
            opcode!(ReadPreImage);
        }
        ("env", "wavm_read_inbox_message") => {
            ty = FunctionType::new(
                vec![MtValueType::I64, MtValueType::I32, MtValueType::I32],
                vec![MtValueType::I32],
            );
            opcode!(LocalGet, 0);
            opcode!(LocalGet, 1);
